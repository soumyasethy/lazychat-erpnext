		...
		"input_schema": {
			...
			"properties": {
				...
				"script": {"type": "string", "description": (
					...
					"If you DO use Script Report, follow these rules:\n"
					"  * NO `import` statements (rejected at preview). `frappe`, `_`, `json` are "
					"pre-injected as globals.\n"
					"  * IN-DB Script Reports (Report.report_script field) DO NOT auto-call `def execute()` "
					"— that's the FILE-based convention. Instead, set TOP-LEVEL variables `columns` "
					"(list of column-def dicts) and `result` (or `data`, list of row dicts). Frappe "
					"extracts these from the safe_exec locals namespace after the script runs.\n"
					...
					"are forbidden.\n"
					"CANONICAL PATTERN (top-level vars, NOT def execute()):\n"
					"  columns = [\n"
					"      {'label': 'Item', 'fieldname': 'item_code', 'fieldtype': 'Link',\n"
					"       'options': 'Item', 'width': 120},\n"
					"  ]\n"
					"  result = frappe.db.get_list('Item', filters=filters or {}, fields=['name', 'item_code'])\n"
					"WRONG (silent null-result bug — function defined but never called):\n"
					"  def execute(filters=None):\n"
					"      return [], []\n"
					"The wrapper AST-validates + safe_exec dry-runs + ACTUALLY EXECUTES the report "
					"at preview time (Cycle 17.3), so any violation — including a `def execute()` "
					"that's never called — surfaces with an actionable hint BEFORE the report ships."
				)},
